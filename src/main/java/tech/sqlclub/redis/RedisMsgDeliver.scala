...
abstract class RedisMsgDeliver extends Serializable with Logging {
  ...
  def sendMsg(toQueue: String, msgStr: String, expireSec: Int): Int = {
    ...
    try {
      val MAX_MSG_LEN = RedisMsgDeliver.conf.getOrElse(Constants.MSG_MAXLENGTH, 1000000).asInstanceOf[Int]
...
object RedisMsgDeliver extends Logging{
  private var msgDeliver :RedisMsgDeliver = null
  private var conf:Map[String,Object] = null

  def getInstance:RedisMsgDeliver = {
    if (msgDeliver == null ){
      this.synchronized {
        if (msgDeliver == null) {
          val config = ConfigUtils.getConfig(Constants.REDIS_DELIVER)
          if (config == null) throw new SQLClubException("can not find redis config")
